...
line_bot_api.push_message(TextSendMessage(text='您好，感謝使用寵物用品商店線上機器人，\n輸入下面指令可以使用本機器人的功能\n---------------------------------------------------\n購買商品\n線上客服\n店家資訊'))
...
def handle_message(event):
    ...
    if re.match('購買商品',message):
        ...
        line_bot_api.push_message(TextSendMessage(text='請稍等，以幫您聯絡客服人員。'))
    elif re.match('店家資訊',message):
        line_bot_api.push_message(TextSendMessage(text='寵物用品專賣門市\n台中市沙鹿區台灣大道七段200號, 433 台中市\n\n電話號碼\n04-26328001\n\n電子郵件\ns1091935@gm.pu.edu.tw\n\n服務時間\n星期一 ~ 星期五： 8:00 - 18:00\n假日：不營業\n特殊節日：另公告之'))
#主程式
import os
